...
3. `facts.md`: prices, bay names, hours, active promo codes. Never state these from memory.
...
- Pick footage only from the asset index, and only rows where the consent flag is true when a face is recognizable.
- Write drafts to `/drafts`. Never mark anything as ready to publish. A person moves work to `/approved`.
- When a draft breaks a rule in the refusal list, stop and say which rule, in one line.
- Anything tagged `proposed` is a default awaiting founder review. Follow it, and mention it if it materially shaped the output.
